...
For single-trait or multi-trait analyses, you can use `construct_fdr_formula()` to automatically place knots where FDR analysis detects signal:
...
bfp_formula <- construct_fdr_formula(sumstats$bfp)
print(bfp_formula)  # Shows adaptive knot placement

# For multiple traits
z <- as.matrix(sumstats[, -1])
multi_formula <- construct_fdr_formula(z)

# construct_fdr_formula() adapts to signal strength per variable:
# - Strong signal (≥100 tests with FDR ≤ 0.5): quartile knots in signal region
# - Moderate signal (50-99 tests): single knot
# - Weak signal (<50 tests): excludes variable from model
```

**Note:** For multi-trait analysis with linkage disequilibrium, the mgcv approach (Approach 1) is still recommended as it is more robust.
